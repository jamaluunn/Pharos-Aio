...
class BrokexModule:
    ...
    async def perform_trade(self, pair_id, is_long, amount):
        ...
        # Logika Baru: Menggunakan ABI untuk memanggil createPendingOrder
        ...
        tx_data = trade_contract.functions.createPendingOrder(
            pair_id,        # assetIndex
            is_long,        # isLong
            trade_amount_wei, # usdSize
            5,              # leverage (hardcoded)
            0,              # slPrice (hardcoded)
            0               # tpPrice (hardcoded)
        )
    ...
    async def run_trading_cycle(self, settings):
        ...
        for i in range(trade_count):
            ...
            is_long = random.choice([True, False]) # Menggunakan boolean True/False
